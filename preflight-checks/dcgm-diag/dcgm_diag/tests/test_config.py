...
class TestConfigFromEnv:
    def test_valid_minimal_config(self, valid_env):
        cfg = Config.from_env()
    ...
    def test_all_options(self, valid_env, monkeypatch):
    ...
    def test_missing_connector_socket(self, clean_env, monkeypatch):
        monkeypatch.setenv("NODE_NAME", "test-node")
        with pytest.raises(ValueError, match="PLATFORM_CONNECTOR_SOCKET is required"):
            Config.from_env()

    def test_missing_node_name(self, clean_env, monkeypatch):
    ...
    def test_invalid_diag_level(self, valid_env, monkeypatch, level):
        monkeypatch.setenv("DCGM_DIAG_LEVEL", level)
        with pytest.raises(ValueError, match="DCGM_DIAG_LEVEL must be 1-4"):
            Config.from_env()

    @pytest.mark.parametrize("level", ["1", "2", "3", "4"])
    def test_valid_diag_levels(self, valid_env, monkeypatch, level):
        monkeypatch.setenv("DCGM_DIAG_LEVEL", level)
        cfg = Config.from_env()
        assert cfg.diag_level == int(level)

    def test_invalid_processing_strategy(self, valid_env, monkeypatch):
