...
# Sends are skipped (no gRPC attempt, no buffering, no cache mutation) when the
# platform-connector Unix socket is absent at send time. Skipped sends are not
# lost: the producer's next poll re-evaluates the underlying condition and
# re-stamps a fresh generatedTimestamp before re-attempting. This counter is
# the operator-visible signal that platform-connector was unavailable on this
# node, distinct from a true gRPC RPC error.
