...
//   - TestMultipleRemediations* and TestHealthEventsAnalyzerProcessingStrategyRuleOverride
//     exercise the MultipleRemediations rule, whose pipeline depends on
//     self-referencing expressions (`this.healthevent.nodename`,
//     `{"$subtract": ["this.healthevent.generatedtimestamp.seconds", ...]}`)
//     combined with a JSONB-boolean match on
//     `healtheventstatus.faultremediated.value`. Empirically the rule does
//     not fire under the PostgreSQL translator today (the Assess step times
//     out because the node condition is never created), so these tests are
//     kept on the MongoDB path until the translator gap is closed.
//
